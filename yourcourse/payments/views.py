...
def initiate_payment(request):
    if request.method == "GET":
        ...
        userContactInfo = UserContact.objects.get(username=current_user.id).contact_number 
        orderAmount = '200'
        orderCurrency = 'INR'
    ...
    if request.method == "POST":
        ...
        if received_data:
            if str(received_data['STATUS']) == "['TXN_SUCCESS']" and received_data['BANKTXNID']:
                ...
                if str(currentUserSubs.subscription) == "NoAccess":
                    currentUserSubs.subscription = UnlimitedAccess
                    currentUserSubs.save()
                elif str(currentUserSubs.subscription) == "LimitedAccess":
                    currentUserSubs.subscription = UnlimitedAccess
                    currentUserSubs.save()
                else:
                    pass
        ...
        context = {
            ...
            'footer': 0,
            'courses': courses,
            'subscription': str(currentUserSubs.subscription),
        }
